...
%   See also VECTOR_SUM_POLAR, VERIFY_LUT_DIRECTIONS, COMPUTE_BAR_SWEEP_RESPONSES
    ...
    % Right-hand rule: 90° clockwise from PD defines the positive ortho direction
    ...
    if ang_diff < 90
        ortho_pos_order = 1:11;   % forward matches right-hand rule
    else
        ortho_pos_order = 11:-1:1; % reverse matches right-hand rule
    end
    ...
    fprintf('Ortho pos_order: %s (right-hand rule, RH target: %.1f°, ortho fwd: %.1f°)\n', ...
